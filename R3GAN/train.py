...
def subprocess_fn(rank, c, temp_dir):
    ...
    # Init torch_utils.
    sync_device = torch.device('cuda', rank) if torch.cuda.is_available() and c.num_gpus > 1 else torch.device('cpu')
...
def main(**kwargs):
    ...
    if opts.preset == 'MRI-128':
        WidthPerStage = [1024, 1024, 512, 256, 128, 128]
        BlocksPerStage = [x for x in [2, 2, 2, 3, 2, 2]]
        CardinalityPerStage = [x for x in [128, 128, 64, 64, 32, 32]]
        FP16Stages = [-1, -2, -3, -4]
        NoiseDimension = 64
